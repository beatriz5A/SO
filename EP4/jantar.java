...
class Mesa { // classe que representa o que está acontecendo na mesa
    ...
    /*
     * método que usa o synchronized para o filósofo (thread) pegar os Hashis um por
     * vez. Recebe o número do filosofo que irá tentar executar esta ação
     */
    public synchronized void pegarHashis(int filosofo) {
        ...
        while (filosofos[aEsquerda(filosofo)] == COMENDO || filosofos[aDireita(filosofo)] == COMENDO) {
            try {
                ...
                /*
                 * após uma tentativa, pausa a execução da thread e espera o retorno do método
                 * "notifyall" para que sua thread volte ao estado de prontidão e entre de novo
                 * na fila para pegar o hashi
                 */
                wait();
    ...
    /*
     * método que usa o synchronized para o filósofo (thread) largar os Hashis um
    ...
    public synchronized void returningHashis(int filosofo) {
        ...
        /*
         * se os filosofos a esquerda ou a direita estiverem com fome, eles terão a
         * chance de comer após o atual filoso largar os hashis
         */
        if (filosofos[aEsquerda(filosofo)] == FOME || filosofos[aDireita(filosofo)] == FOME) {
            notifyAll(); // notifica a thread que está em estado de wait para voltar ao estado de
                         // prontidão
        }
